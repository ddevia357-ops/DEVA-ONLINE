...
function mergeAdminProducts(dbRows){
  // D41: the backend is the source of truth. The old fallback merge could make
  // deleted/disabled built-in products appear again in Admin after refresh.
  return (Array.isArray(dbRows)?dbRows:[]).map(x=>({...x,__fallback:false}))
    .sort((a,b)=>Number(a.product_code||999999)-Number(b.product_code||999999));
}
