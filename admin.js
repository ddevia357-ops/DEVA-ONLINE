...
function editProduct(id){const x=(window.__adminProducts||[]).find(p=>p.id===id);if(!x)return;editingProductId=id;if($('#pcode'))$('#pcode').value=x.product_code||'—';$('#pid').value=x.id;$('#pid').disabled=true;$('#pname').value=x.name||'';$('#pcat').value=x.category||'';$('#pprice').value=x.price_usd||0;$('#pstock').value=Number(x.stock_qty||0);$('#plowstock').value=Number(x.low_stock_threshold??2);$('#pimage').value=x.image||'';$('#pactive').checked=!!x.active;$('#productSaveBtn').textContent='✓ هەڵگرتنی گۆڕانکاری';$('#productResetBtn').hidden=false;$('#productDeleteBtn').hidden=false;if(x.image)$('#productPreview').innerHTML=`<img src="${esc(x.image)}" alt="preview">`;$('#productForm').scrollIntoView({behavior:'smooth',block:'start'})}
...
$('#productForm').onsubmit=async e=>{e.preventDefault();try{const fd=new FormData();const id=editingProductId||$('#pid').value.trim()||slugProduct($('#pname').value)+'-'+Date.now().toString(36);fd.append('id',id);fd.append('name',$('#pname').value.trim());fd.append('category',$('#pcat').value);fd.append('price_usd',$('#pprice').value);fd.append('active',$('#pactive').checked?'true':'false');fd.append('stock_qty',$('#pstock').value||'0');fd.append('low_stock_threshold',$('#plowstock').value||'2');fd.append('image',$('#pimage').value.trim());const file=$('#pfile').files?.[0];if(file)fd.append('productImage',file);const saved=await api('/api/admin/products/upload',{method:'POST',body:fd});note(editingProductId?'بەرهەم نوێکرایەوە':`بەرهەم زیاد کرا — کۆد: ${saved?.product_code||''}`);resetProductForm();load()}catch(x){note(x.message,false)}};
