...
function bindRows(){
  $$('[data-save-price]').forEach(b=>b.onclick=async()=>{const id=b.dataset.savePrice;const x=(window.__adminProducts||[]).find(p=>p.id===id);const price=Number(document.querySelector(`[data-qprice="${CSS.escape(id)}"]`)?.value||0);const old=Number(document.querySelector(`[data-qoldprice="${CSS.escape(id)}"]`)?.value||0);if(!x)return note('بەرهەمەکە نەدۆزرایەوە',false);b.disabled=true;try{const saved=await api('/api/admin/products/'+encodeURIComponent(id)+'/price',{method:'PATCH',body:JSON.stringify({price_usd:price,old_price_usd:old})});x.price_usd=Number(saved?.price_usd ?? price);x.old_price_usd=Number(saved?.old_price_usd ?? old);if(old>0&&Number(saved?.old_price_usd||0)!==old)throw new Error('نرخی پێشوو هەڵنەگیرا — دووبارە هەوڵ بدە');if(saved?.product_code)x.product_code=saved.product_code;window.__adminProducts=(window.__adminProducts||[]).filter((p,i,a)=>a.findIndex(q=>String(q.id)===String(p.id)||(p.product_code&&q.product_code===p.product_code))===i);note(old>price&&price>0?`✓ داشکاندن هەڵگیرا: ${old.toLocaleString()}$ ← ${price.toLocaleString()}$`:'✓ نرخ گۆڕدرا — هەمان بەرهەم، هەمان وێنە و هەمان کۆد');renderAdminProducts(window.__adminProducts||[])}catch(e){note(e.message,false)}finally{b.disabled=false}});
