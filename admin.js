...
function bindRows(){
  ...
  $$('[data-del]').forEach(b=>b.onclick=async()=>{if(!confirm('دڵنیایت ئەم بەرهەمە بە تەواوی بسڕدرێتەوە؟ ئەم کردارە ناگەڕێتەوە.'))return;const id=b.dataset.del;b.disabled=true;try{await api('/api/admin/products/'+encodeURIComponent(id)+'/active',{method:'PATCH',body:JSON.stringify({active:false})});window.__adminProducts=(window.__adminProducts||[]).filter(p=>String(p.id)!==String(id));renderAdminProducts(window.__adminProducts);note('✓ بەرهەمەکە لە وێبسایت و بەڕێوەبردن سڕایەوە');lastLoadAt=0;await load(true)}catch(e){note(e.message,false)}finally{b.disabled=false}});
