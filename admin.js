...
function bindRows(){
  $$('[data-save-price]').forEach(b=>b.onclick=async()=>{const id=b.dataset.savePrice;const x=(window.__adminProducts||[]).find(p=>p.id===id);const price=Number(document.querySelector(`[data-qprice="${CSS.escape(id)}"]`)?.value||0);const old=Number(document.querySelector(`[data-qoldprice="${CSS.escape(id)}"]`)?.value||0);if(!x)return note('بەرهەمەکە نەدۆزرایەوە',false);b.disabled=true;try{await api('/api/admin/products',{method:'POST',body:JSON.stringify({id:x.id,name:x.name,category:x.category,price_usd:price,old_price_usd:old,image:x.image||'',active:x.active!==0,stock_qty:Number(x.stock_qty||0),low_stock_threshold:Number(x.low_stock_threshold??2)})});note('✓ نرخەکە گۆڕدرا و هەڵگیرا');await load()}catch(e){note(e.message,false)}finally{b.disabled=false}});
