...
function resetProductForm(){editingProductId=null;$('#productForm').reset();$('#pactive').checked=true;$('#pstock').value=0;$('#plowstock').value=2;$('#pid').disabled=false;$('#productSaveBtn').textContent='＋ زیادکردنی بەرهەم';$('#productResetBtn').hidden=true;$('#productPreview').innerHTML='<span>🖼️</span><small>وێنەی بەرهەم</small>'}
...
$('#productForm').onsubmit=async e=>{e.preventDefault();try{const fd=new FormData();const id=editingProductId||$('#pid').value.trim()||slugProduct($('#pname').value)+'-'+Date.now().toString(36);fd.append('id',id);fd.append('name',$('#pname').value.trim());fd.append('category',$('#pcat').value);fd.append('price_usd',$('#pprice').value);fd.append('active',$('#pactive').checked?'true':'false');fd.append('stock_qty',$('#pstock').value||'0');fd.append('low_stock_threshold',$('#plowstock').value||'2');fd.append('image',$('#pimage').value.trim());const file=$('#pfile').files?.[0];if(file)fd.append('productImage',file);await api('/api/admin/products/upload',{method:'POST',body:fd});note(editingProductId?'بەرهەم نوێکرایەوە':'بەرهەم زیاد کرا و لە ئەپ دەردەکەوێت');resetProductForm();load()}catch(x){note(x.message,false)}};
