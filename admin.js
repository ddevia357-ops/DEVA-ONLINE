...
async function start(){me=await api('/api/admin/me');csrfToken=me.csrfToken;$('#msg').textContent='';$('#login').hidden=true;$('#panel').hidden=false;$('#who').textContent=`${me.email} · ${me.role}`;$$('.super-only').forEach(x=>x.hidden=me.role!=='SUPER_ADMIN');if(me.role==='STAFF')$('#productForm').hidden=true;$('#statRole').textContent=me.role;$('#stat2fa').textContent=me.totpEnabled?'ON':'OFF';$('#twoFactorState').textContent=me.totpEnabled?'2FA چالاکە':'2FA هێشتا چالاک نەکراوە';$('#setup2fa').hidden=me.totpEnabled;await load()}
async function load(){const [p,o]=await Promise.all([api('/api/admin/products'),api('/api/admin/orders')]);$('#statProducts').textContent=p.length;$('#statOrders').textContent=o.length;$('#statPending').textContent=o.filter(x=>x.status==='PENDING').length;$('#products').innerHTML=p.length?p.map(x=>`<div class="product-admin-row"><img src="${esc(x.image||'')}" alt="" onerror="this.style.visibility='hidden'"><div><b>${esc(x.name)}</b><div class="muted">${esc(x.id)} · ${esc(x.category)} · ${x.active?'Active':'Inactive'}</div></div><span class="product-admin-price">${Number(x.price_usd).toLocaleString()}$</span>${me.role!=='STAFF'?`<div class="product-admin-actions"><button data-edit-product="${esc(x.id)}">دەستکاری</button><button class="danger" data-del="${esc(x.id)}">ناچالاک</button></div>`:''}</div>`).join(''):'<p class="muted">هێشتا هیچ بەرهەمێک لە Dashboard زیاد نەکراوە.</p>';window.__adminProducts=p;$('#orders').innerHTML=o.map(x=>`<div class="row"><div><b>${esc(x.id)}</b><div class="muted">${esc(x.customer_name)} · ${esc(x.phone)} · ${esc(x.payment_status)}${x.discount_code?` · 🎁 ${esc(x.discount_code)} -${Number(x.discount_usd||0).toLocaleString()}$`:''}</div></div><span>${Number(x.total_iqd).toLocaleString()} IQD</span><select data-order="${esc(x.id)}">${['PENDING','CONFIRMED','PREPARING','SHIPPED','DELIVERED','CANCELLED','PAID'].map(s=>`<option ${s===x.status?'selected':''}>${s}</option>`).join('')}</select></div>`).join('');renderDashboard(o);$('#recentOverview').innerHTML=o.slice(0,5).map(x=>`<div class="row"><b>${esc(x.id)}</b><span>${esc(x.status)}</span><span>${esc(x.created_at)}</span></div>`).join('')||'<p class="muted">هیچ داواکارییەک نییە</p>';bindRows()}
function bindRows(){$$('[data-edit-product]').forEach(b=>b.onclick=()=>editProduct(b.dataset.editProduct));$$('[data-del]').forEach(b=>b.onclick=async()=>{try{await api('/api/admin/products/'+encodeURIComponent(b.dataset.del),{method:'DELETE'});note('بەرهەمەکە ناچالاک کرا');load()}catch(e){note(e.message,false)}});$$('[data-order]').forEach(s=>s.onchange=async()=>{try{await api('/api/admin/orders/'+encodeURIComponent(s.dataset.order),{method:'PATCH',body:JSON.stringify({status:s.value})});note('دۆخی داواکاری گۆڕدرا')}catch(e){note(e.message,false)}})}
let editingProductId=null;
function slugProduct(v){return String(v||'product').trim().toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'').slice(0,70)||('product-'+Date.now())}
function resetProductForm(){editingProductId=null;$('#productForm').reset();$('#pactive').checked=true;$('#pid').disabled=false;$('#productSaveBtn').textContent='＋ زیادکردنی بەرهەم';$('#productResetBtn').hidden=true;$('#productPreview').innerHTML='<span>🖼️</span><small>وێنەی بەرهەم</small>'}
function editProduct(id){const x=(window.__adminProducts||[]).find(p=>p.id===id);if(!x)return;editingProductId=id;$('#pid').value=x.id;$('#pid').disabled=true;$('#pname').value=x.name||'';$('#pcat').value=x.category||'';$('#pprice').value=x.price_usd||0;$('#pimage').value=x.image||'';$('#pactive').checked=!!x.active;$('#productSaveBtn').textContent='✓ هەڵگرتنی گۆڕانکاری';$('#productResetBtn').hidden=false;if(x.image)$('#productPreview').innerHTML=`<img src="${esc(x.image)}" alt="preview">`;$('#productForm').scrollIntoView({behavior:'smooth',block:'start'})}
$('#productResetBtn').onclick=resetProductForm;
$('#pfile').onchange=e=>{const f=e.target.files?.[0];if(!f)return;if(!['image/jpeg','image/png','image/webp'].includes(f.type)){e.target.value='';note('تەنها JPG / PNG / WEBP ڕێگەپێدراوە',false);return}if(f.size>8*1024*1024){e.target.value='';note('قەبارەی وێنە نابێت لە 8MB زیاتر بێت',false);return}const reader=new FileReader();reader.onload=()=>{$('#productPreview').innerHTML=`<img src="${reader.result}" alt="preview">`};reader.onerror=()=>note('Preview ـی وێنە دروست نەبوو',false);reader.readAsDataURL(f)};
$('#pimage').oninput=e=>{if(e.target.value.trim())$('#productPreview').innerHTML=`<img src="${esc(e.target.value.trim())}" alt="preview">`};
$('#productForm').onsubmit=async e=>{e.preventDefault();try{const fd=new FormData();const id=editingProductId||$('#pid').value.trim()||slugProduct($('#pname').value)+'-'+Date.now().toString(36);fd.append('id',id);fd.append('name',$('#pname').value.trim());fd.append('category',$('#pcat').value);fd.append('price_usd',$('#pprice').value);fd.append('active',$('#pactive').checked?'true':'false');fd.append('image',$('#pimage').value.trim());const file=$('#pfile').files?.[0];if(file)fd.append('productImage',file);await api('/api/admin/products/upload',{method:'POST',body:fd});note(editingProductId?'بەرهەم نوێکرایەوە':'بەرهەم زیاد کرا و لە ئەپ دەردەکەوێت');resetProductForm();load()}catch(x){note(x.message,false)}};
