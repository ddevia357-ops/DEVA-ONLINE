...
CREATE TABLE IF NOT EXISTS rewards_phone_verifications(id INTEGER PRIMARY KEY AUTOINCREMENT,phone TEXT NOT NULL,token_hash TEXT NOT NULL UNIQUE,expires_at INTEGER NOT NULL,used_at TEXT,ip TEXT,created_at TEXT DEFAULT CURRENT_TIMESTAMP);
...
CREATE TABLE IF NOT EXISTS rewards_entries(id INTEGER PRIMARY KEY AUTOINCREMENT,draw_key TEXT NOT NULL,member_id INTEGER NOT NULL,scanned_at TEXT DEFAULT CURRENT_TIMESTAMP,source TEXT NOT NULL DEFAULT 'SHOWROOM_QR',FOREIGN KEY(member_id) REFERENCES rewards_members(id) ON DELETE CASCADE,UNIQUE(draw_key,member_id));
...
CREATE INDEX IF NOT EXISTS idx_rewards_entries_key ON rewards_entries(draw_key,scanned_at);
...
CREATE INDEX IF NOT EXISTS idx_rewards_phone_verifications_phone ON rewards_phone_verifications(phone,expires_at);
...
// Build 24: professional sponsors + sponsor-backed weekly gifts.
function ensureSponsorRewardsSchemaB24(){
  const add=(table,name,sql)=>{const cols=db.prepare(`PRAGMA table_info(${table})`).all().map(x=>x.name);if(!cols.includes(name)){db.exec(sql);console.log(`[B24] Added ${table}.${name}`)}};
  add('sponsor_ads','logo',"ALTER TABLE sponsor_ads ADD COLUMN logo TEXT DEFAULT ''");
  add('sponsor_ads','images_json',"ALTER TABLE sponsor_ads ADD COLUMN images_json TEXT NOT NULL DEFAULT '[]'");
  add('sponsor_ads','video_url',"ALTER TABLE sponsor_ads ADD COLUMN video_url TEXT DEFAULT ''");
  add('sponsor_ads','instagram',"ALTER TABLE sponsor_ads ADD COLUMN instagram TEXT DEFAULT ''");
  add('sponsor_ads','facebook',"ALTER TABLE sponsor_ads ADD COLUMN facebook TEXT DEFAULT ''");
  add('sponsor_ads','priority',"ALTER TABLE sponsor_ads ADD COLUMN priority INTEGER NOT NULL DEFAULT 0");
  add('sponsor_ads','placements_json',"ALTER TABLE sponsor_ads ADD COLUMN placements_json TEXT NOT NULL DEFAULT '[]'");
  add('monthly_gifts','images_json',"ALTER TABLE monthly_gifts ADD COLUMN images_json TEXT NOT NULL DEFAULT '[]'");
  add('monthly_gifts','sponsor_id',"ALTER TABLE monthly_gifts ADD COLUMN sponsor_id INTEGER");
  add('monthly_gifts','sponsor_text',"ALTER TABLE monthly_gifts ADD COLUMN sponsor_text TEXT DEFAULT ''");
  add('monthly_gifts','sponsor_name',"ALTER TABLE monthly_gifts ADD COLUMN sponsor_name TEXT DEFAULT ''");
  add('monthly_gifts','sponsor_address',"ALTER TABLE monthly_gifts ADD COLUMN sponsor_address TEXT DEFAULT ''");
  add('monthly_gifts','sponsor_phone',"ALTER TABLE monthly_gifts ADD COLUMN sponsor_phone TEXT DEFAULT ''");
  add('rewards_members','phone_verified_at',"ALTER TABLE rewards_members ADD COLUMN phone_verified_at TEXT");
  add('rewards_members','auth_provider',"ALTER TABLE rewards_members ADD COLUMN auth_provider TEXT");
  add('rewards_members','auth_subject',"ALTER TABLE rewards_members ADD COLUMN auth_subject TEXT");
  add('rewards_prizes','provider_type',"ALTER TABLE rewards_prizes ADD COLUMN provider_type TEXT NOT NULL DEFAULT 'DEVA'");
  add('rewards_prizes','sponsor_id',"ALTER TABLE rewards_prizes ADD COLUMN sponsor_id INTEGER");
  add('rewards_prizes','image',"ALTER TABLE rewards_prizes ADD COLUMN image TEXT DEFAULT ''");
  add('rewards_prizes','video_url',"ALTER TABLE rewards_prizes ADD COLUMN video_url TEXT DEFAULT ''");
  add('rewards_prizes','description',"ALTER TABLE rewards_prizes ADD COLUMN description TEXT DEFAULT ''");
}
ensureSponsorRewardsSchemaB24();
db.exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_rewards_members_social ON rewards_members(auth_provider,auth_subject) WHERE auth_provider IS NOT NULL AND auth_subject IS NOT NULL");
