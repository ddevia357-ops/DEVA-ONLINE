...
function mergeAdminProducts(dbRows,tombstones=[]){
  const deletedIds=new Set((tombstones||[]).map(x=>String(x.product_id||'')));
  const deletedCodes=new Set((tombstones||[]).map(x=>String(x.product_code||'')));
  const base=builtinAdminProducts().filter(x=>!deletedIds.has(String(x.id))&&!deletedCodes.has(String(x.product_code||'')));
  const byId=new Map(base.map(x=>[x.id,x]));
  for(const row of Array.isArray(dbRows)?dbRows:[]){if(deletedIds.has(String(row.id))||deletedCodes.has(String(row.product_code||'')))continue;byId.set(row.id,{...byId.get(row.id),...row,__fallback:false});}
...
async function load(force=false){
  ...
  loadInFlight=(async()=>{
    let [dbProducts,o,tombstones]=await Promise.all([api('/api/admin/products'),api('/api/admin/orders'),api('/api/admin/product-tombstones').catch(()=>[])]);const p=mergeAdminProducts(dbProducts,tombstones);$('#statProducts').textContent=p.length;$('#statOrders').textContent=o.length;$('#statPending').textContent=o.filter(x=>x.status==='PENDING').length;$('#statLowStock').textContent=p.filter(x=>x.active&&Number(x.stock_qty)>0&&Number(x.stock_qty)<=Number(x.low_stock_threshold)).length;$('#statOutStock').textContent=p.filter(x=>x.active&&Number(x.stock_qty)<=0).length;window.__adminProducts=p;renderAdminProducts(p);const pcb=$('#productCountBadge');if(pcb)pcb.textContent='('+p.length+')';$('#orders').innerHTML=o.map(x=>`<div class="row"><div><b>${esc(x.id)}</b><div class="muted">${esc(x.customer_name)} · ${esc(x.phone)} · ${esc(x.payment_status)}</div></div><span>${Number(x.total_iqd).toLocaleString()} IQD</span><select data-order="${esc(x.id)}">${['PENDING','CONFIRMED','PREPARING','SHIPPED','DELIVERED','CANCELLED','PAID'].map(s=>`<option ${s===x.status?'selected':''}>${s}</option>`).join('')}</select></div>`).join('');renderDashboard(o);$('#recentOverview').innerHTML=o.slice(0,5).map(x=>`<div class="row"><b>${esc(x.id)}</b><span>${esc(x.status)}</span><span>${esc(x.created_at)}</span></div>`).join('')||'<p class="muted">هیچ داواکارییەک نییە</p>';bindRows();lastLoadAt=Date.now();
  })();
  try{return await loadInFlight}finally{loadInFlight=null}
}

function bindRows(){
  $$('[data-save-price]').forEach(b=>b.onclick=async()=>{const id=b.dataset.savePrice;const x=(window.__adminProducts||[]).find(p=>p.id===id);const price=Number(document.querySelector(`[data-qprice="${CSS.escape(id)}"]`)?.value||0);const old=Number(document.querySelector(`[data-qoldprice="${CSS.escape(id)}"]`)?.value||0);if(!x)return note('بەرهەمەکە نەدۆزرایەوە',false);b.disabled=true;try{const saved=await api('/api/admin/products/'+encodeURIComponent(id)+'/price',{method:'PATCH',body:JSON.stringify({price_usd:price,old_price_usd:old})});x.price_usd=Number(saved?.price_usd ?? price);x.old_price_usd=Number(saved?.old_price_usd ?? old);if(old>0&&Number(saved?.old_price_usd||0)!==old)throw new Error('نرخی پێشوو هەڵنەگیرا — دووبارە هەوڵ بدە');if(saved?.product_code)x.product_code=saved.product_code;const verifyRows=await api('/api/admin/products');const verified=Array.isArray(verifyRows)?verifyRows.find(p=>String(p.id)===String(saved?.id||id)):null;if(verified){x.price_usd=Number(verified.price_usd||0);x.old_price_usd=Number(verified.old_price_usd||0)}if(old>0&&Number(x.old_price_usd||0)!==old)throw new Error('نرخی پێشوو لە Database هەڵنەگیرا');window.__adminProducts=(window.__adminProducts||[]).filter((p,i,a)=>a.findIndex(q=>String(q.id)===String(p.id)||(p.product_code&&q.product_code===p.product_code))===i);note(old>price&&price>0?`✓ داشکاندن هەڵگیرا: ${old.toLocaleString()}$ ← ${price.toLocaleString()}$`:'✓ نرخ گۆڕدرا — هەمان بەرهەم، هەمان وێنە و هەمان کۆد');renderAdminProducts(window.__adminProducts||[])}catch(e){note(e.message,false)}finally{b.disabled=false}});
  $$('[data-availability-product]').forEach(b=>b.onclick=async()=>{const available=b.dataset.available==='1';try{await api('/api/admin/products/'+encodeURIComponent(b.dataset.availabilityProduct)+'/availability',{method:'PATCH',body:JSON.stringify({is_available:!available})});note(available?'بەرهەم کرا بە: بەردەست نییە':'بەرهەم دووبارە بەردەستە');await load(true)}catch(e){note(e.message,false)}});
  $$('[data-edit-product]').forEach(b=>b.onclick=()=>editProduct(b.dataset.editProduct));
  $$('[data-toggle-product]').forEach(b=>b.onclick=async()=>{const id=b.dataset.toggleProduct;const next=b.dataset.active!=='1';b.disabled=true;try{const saved=await api('/api/admin/products/'+encodeURIComponent(id)+'/active',{method:'PATCH',body:JSON.stringify({active:next})});const x=(window.__adminProducts||[]).find(p=>String(p.id)===String(saved?.id||id));if(x)x.active=saved?.active?1:0;note(next?'بەرهەم چالاک کرا':'بەرهەم ناچالاک کرا');lastLoadAt=0;await load(true)}catch(e){note(e.message,false)}finally{b.disabled=false}});
  $$('[data-del]').forEach(b=>b.onclick=async()=>{if(!confirm('دڵنیایت ئەم بەرهەمە بە تەواوی بسڕدرێتەوە؟ ئەم کردارە ناگەڕێتەوە.'))return;const id=b.dataset.del;b.disabled=true;try{await api('/api/admin/products/'+encodeURIComponent(id),{method:'DELETE'});window.__adminProducts=(window.__adminProducts||[]).filter(p=>String(p.id)!==String(id));renderAdminProducts(window.__adminProducts);note('✓ بەرهەمەکە بە تەواوی سڕایەوە و دووبارە ناگەڕێتەوە');lastLoadAt=0;await load(true)}catch(e){note(e.message,false)}finally{b.disabled=false}});
...
function loadNextProductCode(){if(editingProductId)return;const rows=window.__adminProducts?.length?window.__adminProducts:builtinAdminProducts();const max=Math.max(155,...rows.map(x=>Number(String(x.product_code||'').replace(/\D/g,''))||0));if($('#pcode'))$('#pcode').value=String(max+1).padStart(4,'0')}
...
const giftImages=()=>$('#giftImages').value.split(/\n|,/).map(x=>x.trim()).filter(Boolean);const renderGiftGallery=()=>{$('#giftGalleryPreview').innerHTML=giftImages().map((u,i)=>`<div class=\"sponsor-gallery-item\"><img src=\"${esc(u.startsWith('/')?location.origin+u:u)}\"><button type=\"button\" data-giftremove=\"${i}\">×</button></div>`).join('');$$('[data-giftremove]').forEach(b=>b.onclick=()=>{const a=giftImages();a.splice(+b.dataset.giftremove,1);$('#giftImages').value=a.join('\n');renderGiftGallery()})};async function uploadGiftFiles(input,multi=false){const files=[...(input.files||[])];const a=giftImages();for(const f of files){const fd=new FormData();fd.append('media',f);const d=await api('/api/admin/monthly-gifts/upload',{method:'POST',body:fd});if(multi)a.push(d.url);else $('#giftImage').value=d.url}if(multi){$('#giftImages').value=a.join('\n');renderGiftGallery()}input.value=''}$('#giftImageFile')?.addEventListener('change',e=>uploadGiftFiles(e.target));$('#giftGalleryFiles')?.addEventListener('change',e=>uploadGiftFiles(e.target,true));$('#giftImages')?.addEventListener('input',renderGiftGallery);$('#giftForm').onsubmit=async e=>{e.preventDefault();try{await api('/api/admin/monthly-gifts',{method:'POST',body:JSON.stringify({title:$('#giftTitle').value,gift_name:$('#giftName').value,image:$('#giftImage').value,images:giftImages(),sponsor_id:$('#giftSponsorId').value?Number($('#giftSponsorId').value):null,sponsor_text:$('#giftSponsorText').value,description:$('#giftDescription').value,terms:$('#giftTerms').value,start_at:isoLocal($('#giftStart').value),end_at:isoLocal($('#giftEnd').value),status:$('#giftStatus').value})});e.target.reset();renderGiftGallery();note('دیاریی مانگانە زیاد کرا');loadGifts()}catch(x){note(x.message,false)}};
