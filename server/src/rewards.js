...
async function loadMember(){const box=qs('#rewardMyStatus');if(box)box.innerHTML=esc('📱 بەشداریکردن و پشکنینی ئەندامێتی تەنها لە ئەپی DEVA و دوای چوونەژوورەوە بەردەستە.')}
function openJoin(){qs('#rewardJoinModal')?.classList.add('open');const msg=qs('#rewardJoinMsg');if(msg)msg.textContent='🔐 بۆ بەشداریکردن، ئەپی DEVA بکەرەوە و بە Google، Facebook یان Apple بچۆ ژوورەوە.'}
function closeModal(id){qs(id)?.classList.remove('open');if(id==='#rewardJoinModal')stopCamera()}
async function activateWithProfile(){openJoin();return false}
async function activate(){openJoin()}
async function startCamera(){openJoin()}
...
function init(){
 qs('#rewardJoinBtn')?.addEventListener('click',openJoin);qs('#rewardScanBtn')?.addEventListener('click',openJoin);qs('#rewardActivateBtn')?.addEventListener('click',activate);qs('#rewardNotifyBtn')?.addEventListener('click',async()=>{if('Notification'in window)await Notification.requestPermission()});document.querySelectorAll('[data-reward-close]').forEach(b=>b.onclick=()=>closeModal('#'+b.dataset.rewardClose));qs('#mysteryGiftBox')?.addEventListener('click',e=>e.currentTarget.classList.add('opened'));
 const token=new URLSearchParams(location.search).get('deva_reward');if(token){history.replaceState({},'',location.pathname+location.hash);setTimeout(openJoin,300)}
 window.DEVA_REWARDS={handleProfileSaved:async()=>false};
